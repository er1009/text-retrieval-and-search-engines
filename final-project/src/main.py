...
def cmd_run1(args):
    ...
    # Process ALL queries (train + test) for evaluation, then filter for submission
    ...
    print(f"Running BM25+RM3 on {len(all_expanded)} queries (50 train + 199 test)...")
...
def cmd_run2(args):
    ...
    # Initialize THREE-STAGE reranker for MAXIMUM quality:
    # Stage 1: Bi-Encoder (filter 1000 → 500) - fast
    # Stage 2: Cross-Encoder (rerank 500) - medium
    # Stage 3: MonoT5-3B (ensemble) - best quality
    # Total GPU: ~17GB (fits easily on A100 80GB!)
    device = "cuda" if args.gpu else "cpu"
    
    reranker = ThreeStageReranker(
        bi_encoder_model="BAAI/bge-large-en-v1.5",  # Fast bi-encoder for filtering
        ce_model="BAAI/bge-reranker-v2-m3",  # State-of-the-art cross-encoder
        monot5_model="castorini/monot5-3b-msmarco",  # 3B model for quality
        ce_weight=0.5,  # Balanced ensemble
        bi_encoder_top_k=500,  # Filter to top 500 before cross-encoder
        device=device,
        use_bf16=True,
    )
    ...
    for qid in tqdm(all_qids, desc="Neural reranking"):
        ...
        # Three-stage reranking: Bi-Encoder → Cross-Encoder → MonoT5-3B
        # MAXED OUT for A100 80GB: huge batch sizes for GPU saturation!
        passage_scores, _ = reranker.rerank_passages(
            original_query,
            passages,
            bi_batch_size=1024,     # Bi-encoder is VERY fast
            ce_batch_size=1024,     # A100 can handle this!
            monot5_batch_size=256,  # MonoT5-3B with 80GB
        )
...
def main():
    ...
    run2_parser.add_argument(
        "--rerank-depth",
        type=int,
        default=200,  # Rerank more docs for better recall (A100 can handle it!)
