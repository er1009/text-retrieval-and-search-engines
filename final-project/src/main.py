...
def run_pipeline(
    ...
    num_methods = 4 if use_q2d else 2
    
    print("\n" + "=" * 60)
    print(f"  RETRIEVAL: BM25+RM3 + Dense ({num_methods}-way {'Hybrid' if use_q2d else 'Original Only'})")
    print("=" * 60)
    print(f"  Retrieval k={args.retrieval_k} per method")
    if args.disable_q2d:
        print("  ⚠️  Q2D DISABLED (ablation mode)")
    
    print(f"\n  [1/{num_methods}] BM25+RM3 with original queries...")
    ...
    if use_q2d:
        ...
        print("  RUN 1: BM25(orig) + Dense(orig) → RRF (no Q2D)")
    print("=" * 60)
    
    # Fuse retrieval methods
    print(f"  Fusing {num_methods} retrievers with RRF (k={args.rrf_k})...")
    run1_results = {}
    for qid in qids:
        runs = [orig_scores.get(qid), dense_orig.get(qid)]
        if use_q2d:
            runs.extend([exp_scores.get(qid), dense_exp.get(qid)])
    ...
    if args.disable_llm:
        print("  RUN 3: Copy of Run 2 (LLM DISABLED)")
        print("=" * 60)
        print("  ⚠️  LLM DISABLED (ablation mode)")
        run3_results = run2_results.copy()
        write_trec_run_from_scores(run3_results, output_dir / "run_3.res", run_name="Neural_Only")
        print(f"  ✓ Run 3: {len(run3_results)} queries (copy of Run 2)")
    else:
