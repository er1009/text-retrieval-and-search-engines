...
def run_pipeline(
    ...
    # ==================== SHARED: BM25+RM3 RETRIEVAL ====================
    print("\n" + "=" * 60)
    print("  RETRIEVAL: BM25+RM3")
    print("=" * 60)
    
    print("\n  [1/2] BM25+RM3 with original queries...")
    ...
    print("  [2/2] BM25+RM3 with expanded queries (Q2D)...")
    ...
    # ==================== RUN 1: BM25+RM3(orig) + BM25+RM3(Q2D) → RRF ====================
    run1_start = time.time()
    print("\n" + "=" * 60)
    print("  RUN 1: BM25+RM3(orig) + BM25+RM3(Q2D) → RRF")
    print("=" * 60)
    ...
    # ==================== RUN 2: BM25+RM3 + Dense → RRF → Neural ====================
    run2_start = time.time()
    print("\n" + "=" * 60)
    print("  RUN 2: BM25+RM3(orig) + BM25+RM3(Q2D) + Dense → RRF → Neural")
    print("=" * 60)
    
    # Load dense index
    from .dense_index import DensePassageIndex
    
    print(f"\n  Loading dense index from {args.dense_index_path}...")
    dense_index = DensePassageIndex(args.dense_index_path)
    dense_index.load()
    
    print("  Dense retrieval...")
    dense_results = dense_index.batch_search(q_orig, top_k=1000, show_progress=True)
    
    # Fuse BM25(orig) + BM25(Q2D) + Dense with RRF
    print(f"  Fusing BM25+Dense with RRF (k={args.rrf_k})...")
    run2_candidates = {}
    for qid in qids:
        runs = [r for r in [orig_scores.get(qid), exp_scores.get(qid), dense_results.get(qid)] if r]
        if runs:
            run2_candidates[qid] = fuse_runs(runs, method="rrf", rrf_k=args.rrf_k)
    ...
    # Three-way RRF: Run1 + Run2 + LLM
    print(f"\n  Fusing Run1 + Run2 + LLM with RRF (k={args.rrf_k})...")
    run3_results = {}
    for qid in qids:
        runs_to_fuse = []
        ...
        if runs_to_fuse:
            run3_results[qid] = fuse_runs(runs_to_fuse, method="rrf", rrf_k=args.rrf_k)
    
    write_trec_run_from_scores(run3_results, output_dir / "run_3.res", run_name="RRF_LLM")
