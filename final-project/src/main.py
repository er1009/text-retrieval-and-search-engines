...
def run_pipeline(
    ...
    # ==================== RUN 3: LLM(RRF(Run1, Run2)) ====================
    run3_start = time.time()
    print("\n" + "=" * 60)
    print("  RUN 3: LLM(RRF(Run1, Run2)) - Best Practice")
    print("=" * 60)
    
    # Step 1: Fuse Run1 + Run2 first
    print(f"\n  [1/2] Fusing Run1 + Run2 with RRF (k={args.rrf_k})...")
    fused_results = {}
    for qid in qids:
        runs_to_fuse = []
        ...
        if runs_to_fuse:
            fused_results[qid] = fuse_runs(runs_to_fuse, method="rrf", rrf_k=args.rrf_k)
    
    # Step 2: LLM reranks the fused results
    ...
    print(f"\n  [2/2] LLM Reranking on fused top-{args.llm_top_k}:")
    print(f"    Model: {args.llm_model}")
    print(f"    Est. requests: {cost_est['total_requests']}, Est. cost: ${cost_est['estimated_cost_usd']:.2f}")
    
    # Prepare LLM input from fused results (best of Run1 + Run2)
    llm_input = {}
    for qid in qids:
        if qid not in fused_results:
            continue
        sorted_docs = sorted(fused_results[qid].items(), key=lambda x: -x[1])
    ...
    print(f"\n  [LLM] Requests: {llm_reranker.stats.total_requests}, Errors: {llm_reranker.stats.errors}")
    
    # Step 3: Final strong LLM reranking on top-10 (cascade)
    if args.llm_strong_model and args.llm_strong_top_k > 0:
        print(f"\n  [3/3] Final rerank with {args.llm_strong_model} (top-{args.llm_strong_top_k})...")
        
        # Prepare top-k docs for strong LLM
        strong_input = {}
        for qid in qids:
            ...
            docs = batch_get_documents(top_docids)
            strong_input[qid] = [{"docid": d, "text": docs.get(d, "")} for d in top_docids if docs.get(d)]
        
        # Use strong LLM with full listwise (no sliding window needed for 10 docs)
        strong_config = LLMRerankerConfig(
            model=args.llm_strong_model,
            window_size=args.llm_strong_top_k,  # All docs in one window
            step_size=args.llm_strong_top_k,    # Single pass
            max_passage_length=args.llm_max_passage_length,
            max_concurrent_requests=args.llm_concurrency,
        )
        ...
        print(f"  [Strong LLM] Requests: {strong_reranker.stats.total_requests}, Errors: {strong_reranker.stats.errors}")
        
        # Merge: strong LLM top-k + rest from first LLM
        for qid in qids:
            if qid in strong_rankings and strong_rankings[qid]:
                # Strong LLM reordered top-k, keep their scores high
                top_k_reranked = strong_rankings[qid]
                # Add rest from first LLM with lower scores
                if qid in llm_rankings:
                    for docid, score in llm_rankings[qid].items():
                        if docid not in top_k_reranked:
                            top_k_reranked[docid] = score * 0.5  # Scale down
                llm_rankings[qid] = top_k_reranked
    
    # Run 3 = LLM reranked results (with fallback to fused for docs not in LLM output)
    run3_results = {}
    for qid in qids:
        if qid in llm_rankings and llm_rankings[qid]:
            # LLM reranked top-k, keep rest from fused
            run3_results[qid] = llm_rankings[qid].copy()
            # Add lower-ranked docs from fused that LLM didn't see
            if qid in fused_results:
                for docid, score in fused_results[qid].items():
                    if docid not in run3_results[qid]:
                        run3_results[qid][docid] = score * 0.1  # Scale down non-LLM docs
        elif qid in fused_results:
            run3_results[qid] = fused_results[qid]
...
def add_common_args(parser):
    ...
    # Final strong LLM for top-k (cascade reranking)
    parser.add_argument("--llm-strong-model", type=str, default="gpt-5", 
                        help="Strong LLM for final reranking (gpt-5 recommended)")
    parser.add_argument("--llm-strong-top-k", type=int, default=20,
                        help="Number of docs for final strong LLM rerank")
