...
def cmd_run2(args):
    ...
    reranker = ThreeStageReranker(
        ...
        ce_model="cross-encoder/ms-marco-MiniLM-L-6-v2",  # 6 layers, very fast!
        monot5_model="castorini/monot5-base-msmarco",  # Base model (~0.9GB)
        ce_weight=0.5,
        bi_encoder_top_k=1000,  # Keep MORE passages for better recall!
        device=device,
        use_bf16=True,
    )
...
def main():
    ...
    run2_parser.add_argument(
        "--rerank-depth",
        type=int,
        default=500,  # INCREASED: rerank 500 docs for better recall!
