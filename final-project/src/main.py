...
def run_pipeline(
    ...
    if args.use_llm:
        ...
        print(f"\n  [LLM] Stats:")
        print(f"    Requests: {llm_reranker.total_requests}")
        print(f"    Tokens in: {llm_reranker.total_tokens_in:,}")
        print(f"    Tokens out: {llm_reranker.total_tokens_out:,}")
...
def add_common_args(parser):
    ...
    # Neural (bi_encoder_top_k scales with rerank_depth if not specified)
    ...
    parser.add_argument("--llm-concurrency", type=int, default=50, help="Max concurrent LLM requests")
