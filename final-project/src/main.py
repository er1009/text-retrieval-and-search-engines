...
def cmd_run2(args):
    ...
    # BEST PRACTICE: Scale bi_encoder_top_k with rerank_depth
    device = "cuda" if args.gpu else "cpu"
    
    # ~15 passages/doc on average, keep enough through bi-encoder
    estimated_passages = args.rerank_depth * 15
    bi_encoder_top_k = min(estimated_passages, max(3000, args.rerank_depth * 8))
    print(f"Rerank depth: {args.rerank_depth} docs → ~{estimated_passages} passages")
    print(f"Bi-encoder keeps: {bi_encoder_top_k} passages")
    ...
    reranker = ThreeStageReranker(
        ...
        ce_weight=0.5,
        bi_encoder_top_k=bi_encoder_top_k,  # SCALED with input!
    ...
    for qid in tqdm(all_qids, desc="Neural reranking"):
        ...
        for r in bm25_results:
            if r.docid in doc_scores:
                ...
                final_scores[r.docid] = bm25_norm[r.docid] * 0.3
...
def cmd_quick_train(args):
    ...
    for qid in tqdm(train_qids, desc="  Processing"):
        ...
        # Use pure BM25 (scaled down) for unreranked docs
        final_scores = {}
        
        for r in bm25_results:
            if r.docid in doc_scores:
                # Interpolate: neural_weight% neural + (1-neural_weight)% BM25
                final_scores[r.docid] = (
                    neural_weight * doc_scores[r.docid] + 
                    (1 - neural_weight) * bm25_norm[r.docid]
                )
            else:
                # Unreranked: use BM25 score scaled below neural minimum
                final_scores[r.docid] = bm25_norm[r.docid] * (1 - neural_weight)
    ...
    metric_names = ['map', 'ndcg', 'P_10', 'recall_100', 'recall_1000']
    display_names = ['MAP', 'NDCG', 'P@10', 'R@100', 'R@1000']
