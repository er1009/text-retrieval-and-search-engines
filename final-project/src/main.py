...
def run_pipeline(
    ...
    for qid in tqdm(qids, desc="  Reranking"):
        ...
        # Unreranked docs below reranked
        min_reranked = min(final.values()) if final else 0.5
        ceiling = max(0.01, min_reranked * 0.9)
        for r in bm25_results:
            if r.docid not in doc_scores:
                final[r.docid] = ceiling * bm25_norm[qid].get(r.docid, 0)
