...
def cmd_run2(args):
    ...
    # SPEED-OPTIMIZED reranker with smaller models but MORE coverage
    # Smaller models = faster = can rerank MORE docs = better recall!
    device = "cuda" if args.gpu else "cpu"
    
    reranker = ThreeStageReranker(
        bi_encoder_model="BAAI/bge-base-en-v1.5",  # Smaller, fast (~0.4GB)
        ce_model=None,  # Use default ensemble (L6 + L12)
        monot5_model="castorini/monot5-base-msmarco",  # Base model (~0.9GB)
        ce_weight=0.5,
        bi_encoder_top_k=2000,  # MAXIMUM: Keep 2000 passages for best recall!
        use_ensemble_ce=True,  # BEST PRACTICE: Use ensemble of cross-encoders!
        device=device,
        use_bf16=True,
    )
    ...
    all_bm25_results = batch_search_bm25(
        all_expanded,
        k=1000,  # Always get 1000 for recall, rerank top N
        k1=k1, b=b,
    )
    ...
    for qid in tqdm(all_qids, desc="Neural reranking"):
        ...
        # Three-stage reranking with SMALLER models = BIGGER batches = FASTER!
        passage_scores, _ = reranker.rerank_passages(
            original_query,
            passages,
            bi_batch_size=2048,     # Small bi-encoder = huge batches
            ce_batch_size=2048,     # Small cross-encoder = huge batches
            monot5_batch_size=512,  # Base MonoT5 = bigger batches
        )
...
def cmd_quick_train(args):
    ...
    """
    print("=" * 60)
    print("QUICK TRAIN: Fast iteration on 50 training queries")
    print("=" * 60)
    ...
    print(f"Processing {len(train_qids)} training queries (301-350)")
    ...
    print("\n" + "-" * 50)
    print("RUN 1: BM25 + RM3 + Query2Doc")
    print("-" * 50)
    
    # Run with both original and expanded queries, then fuse
    results_orig = batch_search_bm25_rm3(
    ...
    for qid in train_qids:
        ...
        if qid in exp_scores:
            runs_to_fuse.append(exp_scores[qid])
        if runs_to_fuse:
            run1_results[qid] = fuse_runs(runs_to_fuse, method="rrf", rrf_k=60)
    
    write_trec_run_from_scores(run1_results, output_dir / "run_1_train.res", run_name="BM25_RM3_Q2D")
    print(f"✓ Run 1 saved: {len(run1_results)} queries")
    
    # ========== RUN 2: Neural Reranking ==========
    print("\n" + "-" * 50)
    ...
    reranker = ThreeStageReranker(
        ...
        ce_weight=0.5,
        bi_encoder_top_k=2000,
        use_ensemble_ce=True,
        device=device,
        use_bf16=True,
    )
    
    # Batch BM25 retrieval - ALWAYS get 1000 docs for recall, rerank top N
    all_bm25_results = batch_search_bm25(
        train_expanded, k=1000, k1=k1, b=b,
    )
    
    run2_results = {}
    for qid in tqdm(train_qids, desc="Neural reranking"):
        if qid not in all_bm25_results or not all_bm25_results[qid]:
            continue
        ...
        passages, doc_to_indices = get_all_passages_for_query(
            bm25_results, top_k=args.rerank_depth, chunk_size=256, chunk_overlap=64,
        )
        
        if not passages:
            run2_results[qid] = {r.docid: r.score for r in bm25_results[:1000]}
            continue
        
        original_query = train_queries.get(qid, train_expanded.get(qid, ""))
        passage_scores, _ = reranker.rerank_passages(
            original_query, passages,
            bi_batch_size=2048, ce_batch_size=2048, monot5_batch_size=512,
        )
        
        doc_scores = aggregate_passage_scores(passages, passage_scores, doc_to_indices, strategy="maxp")
        
        # Add back lower-ranked docs
        min_score = min(doc_scores.values()) if doc_scores else 0
        for r in bm25_results:
            if r.docid not in doc_scores:
                doc_scores[r.docid] = min_score - (r.rank / 10000)
        
        run2_results[qid] = doc_scores
    
    write_trec_run_from_scores(run2_results, output_dir / "run_2_train.res", run_name="Neural_MaxP")
    print(f"✓ Run 2 saved: {len(run2_results)} queries")
    ...
    for qid in train_qids:
        ...
        if runs_to_fuse:
            run3_results[qid] = fuse_runs(runs_to_fuse, method="rrf", rrf_k=60)
    
    write_trec_run_from_scores(run3_results, output_dir / "run_3_train.res", run_name="RRF_Fusion")
    print(f"✓ Run 3 saved: {len(run3_results)} queries")
    ...
    print("=" * 60)
    print(f"Results saved to: {output_dir}/run_*_train.res")
...
def main():
    ...
    # Quick train command - for fast iteration
    ...
    quick_parser.add_argument(
        ...
        type=int,
        default=200,
        help="Number of documents to rerank (default: 200 for speed)"
    )
    quick_parser.add_argument(
        "--gpu",
        action="store_true",
