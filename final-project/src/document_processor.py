...
from .bm25_retrieval import batch_get_documents
...
def clean_document_text(raw_text: str) -> str:
    """
    Clean ROBUST04 document text by removing XML tags and normalizing whitespace.
    
    ROBUST04 documents contain noisy tags: <H>, <F>, <TEXT>, <FIG>, <DATE>, <TI>, etc.
    Cleaning improves neural reranker and dense index performance significantly.
    """
    if not raw_text:
        return ""
...
def chunk_document(
    ...
    1. Clean XML tags from ROBUST04 documents
    2. Use RecursiveCharacterTextSplitter with semantic separators
    3. Prepend document context to each chunk (Anthropic's approach)
    """
    if not doc_text or not doc_text.strip():
        return []
    
    # Clean XML tags from ROBUST04 documents (HIGH IMPACT)
    doc_text = clean_document_text(doc_text)
    ...
    # Extract context for contextual chunking (from cleaned text)
