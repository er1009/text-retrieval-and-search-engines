...
class CascadeReranker:
    ...
    def rerank(
        ...
        # ═══════════════════════════════════════════════════════════════════
        # FINAL: Interpolate scores (all stages contribute)
        # ═══════════════════════════════════════════════════════════════════
        # Normalize each stage's scores
        bi_norm = self._normalize_dict(bi_doc_scores)
        ce_norm = self._normalize_dict(ce_doc_scores)
        t5_norm = self._normalize_dict(t5_doc_scores)
        
        # Interpolate: docs get their best available score
        # - Docs with T5 score: use T5 (strongest signal)
        # - Docs with CE score (no T5): use CE
        # - Docs with only Bi score: use Bi (weakest)
        final_scores = {}
        for docid in all_docids:
            if docid in t5_norm:
                # Has all 3 stages - interpolate
                final_scores[docid] = (
                    0.1 * bi_norm.get(docid, 0) +
                    0.3 * ce_norm.get(docid, 0) +
                    0.6 * t5_norm[docid]
                )
            elif docid in ce_norm:
                # Has bi + ce - interpolate (scale down since no T5)
                final_scores[docid] = (
                    0.2 * bi_norm.get(docid, 0) +
                    0.5 * ce_norm[docid]
                ) * 0.85  # Slight penalty for not reaching T5
            else:
                # Only bi-encoder score (weakest)
                final_scores[docid] = bi_norm.get(docid, 0) * 0.5
    ...
    def _normalize_dict(self, scores: dict[str, float]) -> dict[str, float]:
        """Normalize score dictionary to [0, 1]."""
        if not scores:
            return {}
