...
class ThreeStageReranker:
    ...
    def __init__(
        ...
        ce_model: str = "cross-encoder/ms-marco-MiniLM-L-6-v2",  # 6-layer, fast!
        monot5_model: str = "castorini/monot5-base-msmarco",  # Base model
        ce_weight: float = 0.5,
        bi_encoder_top_k: int = 1000,  # Keep MORE for better recall!
        device: str = "cuda",
        use_bf16: bool = True,
    ):
        print("=" * 60)
        print("Loading THREE-STAGE RERANKER (SPEED OPTIMIZED)")
        print("  Stage 1: BGE-base Bi-Encoder (fast)")
        print("  Stage 2: MiniLM-L6 Cross-Encoder (fast)")
