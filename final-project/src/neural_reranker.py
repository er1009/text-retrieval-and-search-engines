...
class ThreeStageReranker:
    ...
    def __init__(
        ...
        monot5_model: str = "castorini/monot5-3b-msmarco",  # 3B model for A100!
        ce_weight: float = 0.5,
        ...
        print("=" * 60)
        print("Loading THREE-STAGE RERANKER (UPGRADED MODELS)")
        print("  Stage 1: BGE-base Bi-Encoder (fast)")
        if use_ensemble_ce:
            print("  Stage 2: Cross-Encoder Ensemble (BGE-reranker-v2-m3 + L12)")
        else:
            print("  Stage 2: Cross-Encoder")
        print(f"  Stage 3: {monot5_model.split('/')[-1]} (ensemble)")
        print("=" * 60)
        ...
        # UPGRADED: BGE-reranker-v2-m3 is SOTA on BEIR benchmarks
        if use_ensemble_ce:
            if ce_model is None:
                ce_models = [
                    "BAAI/bge-reranker-v2-m3",  # SOTA reranker (568M params)
                    "cross-encoder/ms-marco-MiniLM-L-12-v2",  # Fast complementary
                ]
            else:
    ...
    def rerank_passages(
        ...
        ce_batch_size: int = 256,     # BGE-reranker-v2-m3 (568M) needs smaller batches
        monot5_batch_size: int = 64,  # MonoT5-3B needs smaller batches on A100
    ) -> tuple[list[float], list[int]]:
        """
