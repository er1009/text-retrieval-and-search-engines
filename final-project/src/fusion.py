...
def rrf(
    ...
    RRF formula: score(d) = sum(w_i / (k + rank_r(d)))
    ...
    Args:
        ...
        weights: Optional weights for each run (default: equal weights)
    ...
    fused = {}
    for rank_run, weight in zip(rank_runs, weights):
        for docid, rank in rank_run.items():
            if docid not in fused:
                fused[docid] = 0.0
            fused[docid] += weight / (k + rank)
...
def fuse_runs(
    ...
    if method == "rrf":
        return rrf(runs, k=rrf_k, weights=weights)
    elif method == "combsum":
        return combsum(runs)
    elif method == "combmnz":
        return combmnz(runs)
