...
class DensePassageIndex:
    ...
    def exists(self) -> bool:
        ...
        # Validate config matches current settings
        try:
            with open(self.config_path) as f:
                saved_config = json.load(f)
            
            if saved_config.get("embedding_model") != self.embedding_model_name:
                print(f"  [Index] Config mismatch: model {saved_config.get('embedding_model')} != {self.embedding_model_name}")
                return False
            if saved_config.get("chunk_size") != self.chunk_size:
                print(f"  [Index] Config mismatch: chunk_size {saved_config.get('chunk_size')} != {self.chunk_size}")
                return False
    ...
    def build(self, batch_size: int = 2048, show_progress: bool = True):
        ...
        # batch_size=512 is optimal for A100 GPU with BGE-base
        encode_batch_size = min(batch_size, 512)
        print(f"  Encoding {len(texts):,} passages (batch_size={encode_batch_size})...")
        ...
        # Use IVF for faster search (train on subset if large)
        nlist = min(4096, num_passages // 40)  # clusters
        quantizer = faiss.IndexFlatIP(dim)  # Inner product (for normalized vectors = cosine)
        index = faiss.IndexIVFFlat(quantizer, dim, nlist, faiss.METRIC_INNER_PRODUCT)
        
        # Train on subset if too large
        if num_passages > 100000:
            train_size = min(100000, num_passages)
    ...
    def search(
        ...
        scores, indices = self._index.search(query_embedding, top_k * 3)  # Get more passages for MaxP
    ...
    def batch_search(
        ...
        # Search
        self._index.nprobe = nprobe
        all_scores, all_indices = self._index.search(query_embeddings, top_k * 3)
