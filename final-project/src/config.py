...
class BM25Config:
    ...
    # BM25 parameters - ROBUST04 published optimal: k1=0.82, b=0.68
    k1: float = 0.82
    b: float = 0.68
    ...
    # Grid search ranges (includes published optimal values)
    k1_range: list[float] = field(default_factory=lambda: [0.6, 0.7, 0.82, 0.9, 1.0, 1.2])
    b_range: list[float] = field(default_factory=lambda: [0.4, 0.5, 0.6, 0.68, 0.75, 0.8])
...
class ChunkingConfig:
    ...
    # 384 chars provides better context for cross-encoders while staying under token limits
    chunk_size: int = 384
    chunk_overlap: int = 96  # ~25% overlap for continuity
...
class NeuralConfig:
    ...
    bi_encoder_top_k: int = 2000  # INCREASED: Keep 2000 passages for better recall!
    ...
    # Reranking depth - MAXIMUM for best recall!
    rerank_depth: int = 1000  # Rerank top 1000 docs for maximum coverage!
...
class AggregationConfig:
    ...
    # Strategy: 'maxp', 'sump', 'firstp', 'topkp', 'hybrid'
    # 'hybrid' is best for news articles (ROBUST04): 70% MaxP + 30% FirstP
    strategy: str = "hybrid"
